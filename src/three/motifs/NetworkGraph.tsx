...
export const NetworkGraph = ({
  ...
  const packetGeo = useMemo(() => {
    ...
    g.setAttribute("aPhase", new BufferAttribute(new Float32Array(total), 1));
    return g;
  }, []);
  ...
  useFrame((state, delta) => {
    ...
    const presence = presenceAt(flight.current.t, WAYPOINT.skills);
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      node.x = node.homeX + f.x * GRAPH_CONFIG.driftAmplitude;
      node.y = node.homeY + f.y * GRAPH_CONFIG.driftAmplitude;
      node.z = node.homeZ + f.z * GRAPH_CONFIG.driftAmplitude;
    ...
    for (let i = 0; i < edges.length; i++) {
      ...
      const lit = e.litUntil > time ? EDGE_LIT_ALPHA : EDGE_BASE_ALPHA;
      eAlpha[i * 2] = lit;
      eAlpha[i * 2 + 1] = lit;
    }
    ...
    for (let i = 0; i < router.pool.length; i++) {
      ...
      for (let k = 0; k < PACKET_CONFIG.trailLength; k++) {
        ...
        pScale[slot] =
          p.size * PACKET_CONFIG.trailAlpha * (1 - k / PACKET_CONFIG.trailLength);
      }
    }
